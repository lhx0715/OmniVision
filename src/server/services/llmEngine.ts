...
const TimelineSchema = z
  .object({
    events: z
      ...
      .min(5)
      .max(20)
      .describe('5-20个核心转折点：尽可能覆盖所有重要时间节点，数量必须充足以支持多页时间线展示'),
  })
  .passthrough()
...
8. **时间线数量**（强制）：timeline.events **至少返回 5 条，最多 20 条**。尽可能覆盖起点、成长、关键转折、巅峰、挫折、现状等所有重要节点，不能只返回 3-4 条草草了事。信息源越丰富，时间线越要丰满。
