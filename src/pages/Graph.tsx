...
import {
  ...
  Circle, Share2, Users, Zap, Search,
  Layers, Eye, EyeOff, ShieldBan, X,
} from 'lucide-react';
...
const RELATION_COLORS: Record<string, string> = {
  ...
  darkside: '#f43f5e',
  timeline: '#06b6d4',
  llm: '#f59e0b', // LLM 语义抽取的关系（人-人/人-事等）
};
...
function EntityNode({ data }: { data: any }) {
  ...
  const size = data.isFocused
    ? 60
    : data.dim
      ? Math.max(baseSize - 8, 18)
      : baseSize;
  ...
  return (
    <div
      ...
      style={{
        width: size,
        height: size,
        borderColor: data.isFocused
          ? color
          : data.dim
            ? `${color}20`
            : `${color}60`,
        backgroundColor: data.isFocused
          ? `${color}30`
          : data.dim
            ? `${color}04`
            : `${color}08`,
    ...
    >
      ...
      {showLabel ? (
        <span
          className="font-mono text-zinc-100 truncate max-w-[140px] px-1 text-center"
          style={{ fontSize: data.isFocused ? 11 : data.isRoot ? 10 : 8 }}
        >
          {data.label}
        </span>
      ) : null}
...
export default function Graph() {
  ...
  const [depth, setDepth] = useState<DepthMode>('1');
  ...
  // 选中节点（由 focusedNodeId 派生，供浮动卡片使用）
  ...
  const visibleIds = useMemo(() => {
    ...
    // 聚焦簇强制可见
    if (focusNodeIds) for (const id of focusNodeIds) baseReachable.add(id);
  ...
  const rfNodes: Node[] = useMemo(() => {
    // 聚焦模式：聚焦节点居中，邻居辐射环绕；非聚焦节点保持环形原位
  ...
  }, [visibleNodes, focusedNodeId, focusCluster, allNodes, focusNodeIds, visibleIds]);

  // ===== 边：全局视图不显示连线；聚焦时仅渲染主节点↔关联节点的高亮关系边 =====
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs font-mono text-zinc-600 tracking-wider">构建认知网络...</p>
        </div>
      </div>
    );
  }
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-white/[0.03]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-mono text-sm tracking-[0.25em] text-zinc-200 uppercase">知识图谱</h1>
          </div>
        </div>

        {/* 统计 */}
        <div className="flex items-center gap-4 text-[10px] font-mono text-zinc-600 tracking-wider uppercase">
      ...
      <main className="relative z-10" style={{ height: 'calc(100vh - 65px)' }}>
        {allNodes.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            {loadError ? (
              <>
                <ShieldBan className="h-16 w-16 text-rose-500/40 mb-4" />
                <p className="font-mono text-sm text-rose-400/70 tracking-wider mb-1">图谱加载受限</p>
                <p className="text-xs text-zinc-600 max-w-xs">{loadError}</p>
                <button
                  onClick={loadGraph}
                  className="mt-4 rounded-lg border border-violet-500/30 bg-violet-500/10 px-3 py-1.5 text-xs text-violet-300 hover:bg-violet-500/20 transition-colors font-mono"
                >
                  重试
                </button>
              </>
              ...
              <>
                <Network className="h-16 w-16 text-zinc-700 mb-4" />
                <p className="font-mono text-sm text-zinc-500 tracking-wider mb-1">图谱尚未生成</p>
                <p className="text-xs text-zinc-600 max-w-xs">
                  收藏包含博弈关系（gameplay）的卡片后，图谱将自动从卡片中提取实体与关系
                </p>
                <button
                  onClick={() => navigate('/')}
                  className="mt-4 rounded-lg border border-violet-500/30 bg-violet-500/10 px-3 py-1.5 text-xs text-violet-300 hover:bg-violet-500/20 transition-colors font-mono"
          ...
          <div className="relative h-full" ref={containerRef}>
            ...
            >
              ...
              <Controls
                className="!bg-zinc-950/90 !border-white/20 [&_button]:!bg-zinc-900/80 [&_button]:!border-white/20 [&_button]:!text-white [&_button]:!hover:bg-zinc-800/80 [&_button]:!hover:text-violet-300"
              />
              <MiniMap
            ...
            {selectedNode && (
              <div className="absolute top-4 left-4 z-30 w-64 rounded-xl border border-violet-500/25 bg-zinc-950/95 backdrop-blur-xl p-4 shadow-[0_0_40px_rgba(139,92,246,0.12)] animate-auth-dialog-in">
                ...
                <div className="flex items-center gap-3 mb-3 text-[10px] font-mono text-zinc-500">
                  ...
                  {selectedNode.sourceQuery && (
                    <span className="flex items-center gap-1 truncate">
                      <Zap className="h-3 w-3" />
                      <span className="truncate">{selectedNode.sourceQuery}</span>
                    </span>
                  )}
                </div>
                ...
                {selectedConnectedEdges.length > 0 ? (
                  <div className="pt-2 border-t border-white/[0.06]">
                      ...
                      {selectedConnectedEdges.map((e) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className="font-mono shrink-0 ml-auto px-1.5 py-0.5 rounded"
                    ...
                    <p className="mt-2 text-[9px] font-mono text-zinc-700 tracking-wider">
                      点击关系项可沿链聚焦 →
                    </p>
                  </div>
                ) : (
                  <p className="text-[10px] text-zinc-600 pt-2 border-t border-white/[0.06]">
                    暂无关联关系
                  </p>
                )}
                ...
                <button
                  ...
                  className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-[11px] font-mono text-cyan-300 hover:bg-cyan-500/20 hover:border-cyan-500/50 transition-all tracking-wider"
                >
                  <Search className="h-3 w-3" />
                  深度情报检索
                </button>

                {/* 以此为起点探索（PRD-02 FR-09：从收藏图谱导入种子）*/}
                <button
                  onClick={() => {
                    const params = new URLSearchParams({
                      seedLabel: selectedNode.label,
                      from: 'graph_node',
                    });
                    if (selectedNode.entityType) params.set('seedType', selectedNode.entityType);
                    navigate(`/explore?${params.toString()}`);
                  }}
                  className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-violet-500/30 bg-violet-500/10 px-3 py-2 text-[11px] font-mono text-violet-300 hover:bg-violet-500/20 hover:border-violet-500/50 hover:shadow-[0_0_16px_rgba(139,92,246,0.25)] transition-all tracking-wider"
                >
            ...
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2">
              ...
              <div className="relative">
                <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-zinc-950/80 backdrop-blur-sm px-3 py-1.5 w-72 focus-within:border-violet-500/40 focus-within:shadow-[0_0_20px_rgba(139,92,246,0.15)] transition-all">
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[9px] font-mono text-zinc-600 shrink-0">
                                {ENTITY_LABELS[n.entityType || 'null']}
                              </span>
                              <span
              ...
              <div className="flex items-center gap-1 rounded-lg border border-white/10 bg-zinc-950/80 backdrop-blur-sm px-2 py-1.5">
                ...
                {(['1', '2', 'all'] as DepthMode[]).map((d) => (
                  <button
                    ...
                    className={cn(
                      'rounded px-2 py-0.5 text-[10px] font-mono tracking-wider transition-colors',
                      depth === d
            ...
            <div className="absolute top-4 right-4 z-20 w-72 rounded-xl border border-white/10 bg-zinc-950/95 backdrop-blur-xl p-4 shadow-xl">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-[11px] font-mono text-zinc-300 tracking-wider uppercase">
                  分类汇总
                </h3>
              </div>
            ...
            <div className="absolute bottom-4 left-4 z-20 rounded-lg border border-white/10 bg-zinc-950/80 backdrop-blur-sm p-3 space-y-2 min-w-[140px]">
              <p className="text-[9px] font-mono text-zinc-600 tracking-widest uppercase mb-1">
                实体类型筛选
              </p>
              {entityTypes.map((type) => {
                ...
                return (
                  <label
                    key={type}
                    className="flex items-center gap-2 text-[10px] text-zinc-400 cursor-pointer hover:text-zinc-200 transition-colors"
                  >
                    <button
                    ...
                    <span className={cn(isHidden && 'text-zinc-600 line-through')}>
                      {ENTITY_LABELS[type]}
                    </span>
                    <span className="ml-auto font-mono text-zinc-600">
                      {allNodes.filter((n) => (n.entityType || 'null') === type).length}
                    </span>
                  </label>
                );
              })}
              ...
              <div className="flex items-center gap-2 text-[10px] text-zinc-400">
                <span className="h-0.5 w-4 bg-rose-500" style={{ borderTop: '1px dashed #f43f5e' }} />
                争议
              </div>
              <div className="flex items-center gap-2 text-[10px] text-zinc-400">
        ...
        <NodeDetailDialog
          ...
          entityType={selectedNode?.entityType || null}
        />
      </main>
    </div>
  );
